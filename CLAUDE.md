...
**Gemini 503s are per-model quotas, not one Gemini outage.** The key's dashboard (15-sep) showed 3.1-flash-lite jammed (RPM 6/15, TPM 44k/250k, **RPD 38/500**) while its `-latest` alias shares all three pools and 3.5-flash-lite sat at 0/500. So every schema call goes through `gemini_worker.generate_with_capacity_chain` — the stages, the worker's own passes, the layout/hook/screencast frame calls: `GEMINI_STAGE_RETRIES` jittered attempts (default 6, ~7 min) then a **family jump** (`GEMINI_MODEL_FALLBACKS`, default = the whole AI Studio text lineup for the pipeline model — 3.5-flash-lite → 2.5-flash-lite → 3-flash → 3.5-flash → 2.5-flash, all priced in `clip_selection.MODEL_PRICES`; a custom primary gets no surprise siblings, and the env var replaces the default outright). A `429 RESOURCE_EXHAUSTED` is the DAILY (RPD) pool — spent until midnight Pacific — so it cools for an hour (`GEMINI_MODEL_EXHAUSTED_COOLDOWN_S`), not the capacity-spike 10 minutes; re-trying a spent model every 10 minutes would burn the whole chain's day away. Retrying a jammed model with a bigger budget is not enough when a per-minute or daily cap is what tripped; and a 429/exhaustion looks like a 503 to the caller, so it rides the same chain. Non-transient failures (400s, policy blocks) still raise immediately.
